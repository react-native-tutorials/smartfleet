...
public class UserPanel extends JPanel {
	...
	static final String NEW_CMD = "NEW_USER";
	static final String ED_CMD = "EDIT_USER";
	static final String SAV_CMD = "SAVE_USER";
	static final String DEL_CMD = "DEL_USER";
	 ...
	 */
	public UserPanel(AbstractRefreshableApplicationContext ctx) {
	...
	public JTable getMainTable() {
		if (mainTable == null) {
			...
			List<Usuario> allDevices = facade.findAllUsers();
			model = new UserTableModel(allDevices);
	...
	public void createItem() {
		...
		EditUserUI det = new EditUserUI(user, this);
	...
	public void editItem() {
		...
		if (row > -1) {
			UserTableModel aModel = (UserTableModel) getMainTable().getModel();
			final Long id = (Long) aModel.getValueAt(row, 0);
			final String docUser = (String) aModel.getValueAt(row, 1);
			final String nomUser = (String) aModel.getValueAt(row, 2);
			final String apeUser = (String) aModel.getValueAt(row, 3);
			final String accUser = (String) aModel.getValueAt(row, 4);
			final String passUser = (String) aModel.getValueAt(row, 5);
			final String actUser = (String) aModel.getValueAt(row, 6);

			Usuario userDetail = new Usuario();
			userDetail.setId(id);
			userDetail.setUsuDoc(docUser);
			userDetail.setUsuNom(nomUser);
			userDetail.setUsuApe(apeUser);
			userDetail.setUsuAcc(accUser);
			userDetail.setUsuPass(passUser);
			userDetail.setActivo(actUser);
			
			EditUserUI det = new EditUserUI(userDetail, this);
			det.setLocationRelativeTo(this);
			det.setModal(Boolean.TRUE);
			det.setVisible(Boolean.TRUE);
		} else {
	...
	public void deleteItem() {
		...
		if (row > -1) {
			...
			if (JOptionPane.YES_OPTION == value) {
				Long userId = Long.parseLong(getMainTable().getValueAt(row, 0).toString());
				facade.deleteUser(userId);
				UserTableModel userTableModel = (UserTableModel) getMainTable().getModel();
