...
public class ShowGpsPanel extends JPanel {

	/**
	 * generated serial version
	 */
	private static final long serialVersionUID = 3223626440475679854L;

	private double[][] coords;
	private double[][] coordsScaled;
	private final int maxLatitudeInPixels = 1500;
	...
	private void scaleCoords() {
		...
		for (int i = 0; i < coords.length; i++) {
			...
			double distanceSquared = latitude * latitude + longitude * longitude;
	...
	protected void paintComponent(Graphics g) {
		...
		for (int i = 0; i < coordsScaled.length; i++) {
			...
			Ellipse2D.Double point = new Ellipse2D.Double(newLatitude, newLongitude, 5, 5);
			String original = "Original: " + originalLatitude + "," + originalLongitude;
			String scaled = "Scaled: " + newLatitude + "," + newLongitude;

			float originalStringX = (float) (newLatitude - fm.stringWidth(original));
